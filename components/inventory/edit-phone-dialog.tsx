...
import { updatePhone } from "@/app/actions/phones";
...
export function EditPhoneDialog({ phone, trigger }: EditPhoneDialogProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    boughtPrice: phone.boughtPrice.toString(),
    sellingPrice: phone.sellingPrice.toString(),
    dateBought: new Date(phone.dateBought).toISOString().split("T")[0],
    images: phone.images,
  });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      await updatePhone(phone.id, session.user.id, {
        name: formData.name,
          ...
          <div className="space-y-2">
            ...
            <div className="rounded-lg border border-dashed p-4 bg-muted/50">
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                Click to upload more images (max 10, 4MB each)
              </p>
            </div>
